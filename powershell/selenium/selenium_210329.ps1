...
foreach ($element in $ffDriver.findElementsByClassName("tF2Cxc")) {
 Write-Host ("◆ " + $element.findElementByClassName("LC20lb").Text + " -> " + $element.FindElementByTagName("a").GetAttribute("href"))
 Write-Host $element.findElementByClassName("aCOpRe").Text
 Write-Host
}
